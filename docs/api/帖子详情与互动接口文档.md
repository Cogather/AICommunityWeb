...
| `/api/posts/{id}/featured` | PUT | 设置帖子精华/置顶状态（管理员） |
...
管理员在帖子详情页面点击"加精"或"置顶"按钮时调用。不同区域有不同的功能名称：
- AI优秀实践：精华
- 赋能交流：精华
- 扶摇Agent应用：置顶
- AI工具专区-其他工具：精华
...
| 区域 | zone值 | toolId | 功能名称 | 数量限制 |
|------|--------|--------|---------|---------|
| AI优秀实践 | practices | null | 精华 | 无限制 |
| 赋能交流 | empowerment | null | 精华 | 无限制 |
| 扶摇Agent应用 | agent | -1 | 置顶 | **只能1个** |
| AI工具专区-其他工具 | tools | 0 | 精华 | 无限制 |
| AI工具专区-普通工具 | tools | 1,2,... | 不支持 | - |

> **⚠️ 扶摇Agent应用限制**：扶摇Agent应用页面只能同时存在一个置顶帖子。如果已有置顶帖子，需要先取消现有置顶后才能设置新的置顶帖子。
...
| replies[].replyToId | number | 被回复的评论/回复ID（用于前端定位展示） |
...
| replyToId | number | 否 | 被回复的评论/回复ID（用于前端定位展示） |
